-- ПАТЧ №19 — Полный фикс многоэтапной «Покупки»: этапы, таймеры, UTC, один msg-таймер
-- Обновление базы данных для поддержки новых полей

-- 1. Добавляем новые поля в таблицу deals
...
-- 2. Добавляем новые поля в таблицу deal_rounds
ALTER TABLE deal_rounds ADD COLUMN is_active BOOLEAN DEFAULT 0 NOT NULL;

-- 3. Добавляем новые поля в таблицу system_settings
